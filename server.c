...
static struct json_object * map_columns_to_indexes(
    unsigned int request_columns_amount, char ** request_columns_names,
    ...
    if (request_columns_amount == 0) {
        ...
        for (unsigned int i = 0; i < columns_count; ++i) {
            ...
            if (!found) {
                ...
                char msg[msg_length];
                snprintf(msg, msg_length, "column with name %s does not exist in table", request_columns_names[i]);
...
static struct json_object * check_values(
    unsigned int request_values_amount,
    struct storage_value ** request_values_values,
...
static struct json_object * handle_request_insert(struct json_api_insert_request request, struct storage * storage) {
...

    {
     ...
     // проверка типов и количества вставляемых значений
    {
    ...
    // выделяем место в файле под строку и значения
...
static struct json_object * is_where_correct(struct storage_joined_table * table, struct json_api_where * where) {
    ...
    switch (where->op) {
        ...
        case JSON_API_OPERATOR_GE:
            ...
            for (unsigned int i = 0; i < table_columns_amount; ++i) {
                ...
                if (strcmp(column.name, where->column) == 0) {
                    ...
                    // формирование сообщения об ошибке. Несоответствие типов колонки и значения
            ...
            // формирование сообщения об ошибке. Нет такой колонки
            {
        ...
        {
            struct json_object * left_error = is_where_correct(table, where->left);
            if (left_error != NULL) {
                return left_error;
            }
...
static bool eval_where(struct storage_joined_row * row, struct json_api_where * where) {
    ...
    switch (where->op) {
        ...
        case JSON_API_OPERATOR_LE:
        case JSON_API_OPERATOR_GE:
            // выбираем нужную колонку и сравниваем значения
            for (unsigned int i = 0; i < table_columns_amount; ++i) {
            ...
            errno = EINVAL;
            return false;
        // уходим в рекурсию, возвращаем результат операции && для левой и правой части
        case JSON_API_OPERATOR_AND:
...
static struct json_object * handle_request_delete(struct json_api_delete_request request, struct storage * storage) {
    ...
    // ищем подходящие строки и удаляем их
...
static struct json_object * handle_request_select(struct json_api_select_request request, struct storage * storage) {
    ...
    for (int i = 0; i < request.joins.amount; ++i) {
        ...
        // добавляем индекс колонки, по которой объединяем, для той таблицы, которую присоединяем
        ...
        // добавляем индекс колонки из уже объединённой таблицы, по которой объединяем, для той таблицы, которую присоединяем
    ...
    // какие колонки должны быть выбраны селектом
    {
        struct json_object * error = map_columns_to_indexes(request.columns.amount, request.columns.columns,
            joined_table, &columns_amount, &columns_indexes);
...
static struct json_object * handle_request_update(struct json_api_update_request request, struct storage * storage) {
    ...
    // формируем ответ с количеством изменённых строк
    struct json_object * answer = json_object_new_object();
    json_object_object_add(answer, "amount", json_object_new_uint64(amount));
    return json_api_make_success(answer);
}
...
int main(int argc, char * argv[]) {
    ...
    // define port
    uint16_t port = argv[2] ? (uint16_t) atoi(argv[2]) : 9001;
