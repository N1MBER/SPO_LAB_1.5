...
void storage_table_remove(struct storage_table * table) {
    ...
    // для удаления последней добавленной таблицы
    if (pointer == 0) {
        pointer = 4;
...
struct storage_row * storage_table_add_row(struct storage_table * table) {
    ...
    // обновляем первую строку таблицы
...
struct storage_value * storage_row_get_value(struct storage_row * row, uint16_t col_index) {
    if (col_index >= row->table->columns.amount) {
        errno = EINVAL;
        return NULL;
    }

    // выбираем нужную ячейку
    lseek64(row->table->storage->fd, (off64_t) (row->position + (1 + col_index) * sizeof(uint64_t)), SEEK_SET);
    ...
    value->type = row->table->columns.columns[col_index].type;
...
static bool storage_joined_row_is_in(struct storage_joined_row * row, uint16_t table_index) {
    return storage_value_is_equals(
        storage_joined_row_get_value(row, row->table->tables.tables[table_index].s_column_index),
        storage_row_get_value(row->rows[table_index], row->table->tables.tables[table_index].t_column_index)
    );
}

static void storage_joined_row_roll(struct storage_joined_row * row) {
    for (int i = 1; i < row->table->tables.amount; ++i) {
        if (!storage_joined_row_is_in(row, i)) {
...
struct storage_value * storage_joined_row_get_value(struct storage_joined_row * row, uint16_t col_index) {
    for (int i = 0; i < row->table->tables.amount; ++i) {
        if (col_index < row->table->tables.tables[i].table->columns.amount) {
            return storage_row_get_value(row->rows[i], col_index);
        }

        col_index -= row->table->tables.tables[i].table->columns.amount;
    }
